...
| **Filesystem logic** | target discovery, `node_modules` state, hardlink/symlink behaviour, strategy availability | build a tree with `tempfile`, assert | Yes — primary |
...
3. Creating a worktree with `hardlink` shares inodes (`stat -c '%i'`) and the result
   builds.
...
6. On a repo with no `package.json`, the modules column and the form's modules row are
   both absent.
